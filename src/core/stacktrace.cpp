...
static auto concatStr(const char* str, char* dest, const char* destEnd) -> char* {
	if(dest >= destEnd) {
		return dest;
	}
...
auto dumpWin(char* buf, int size) -> void {
	...
	SYMBOL_INFO *symbol = (SYMBOL_INFO *)malloc(sizeof(SYMBOL_INFO)+(max - 1) * sizeof(TCHAR));
...
#else
auto dumpUnix(char* buf, int size) -> void {
	buf[0] = 0;
	constexpr static int max = 64;
	void* frames[max];
...
auto dump(char* buf, int size) -> void {
#ifdef _WIN32
	dumpWin(buf, size);
#else
	dumpUnix(buf, size);
